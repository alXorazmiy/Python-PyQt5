...
class LoginPage(QWidget):
    ...
    def on_click(self, event):
        ...
        if username == "admin" and password == "admin":
                self.MyApp.notification.set_message("Successfully", "message", "#1b9c85", "success")
                self.MyApp.show_notification()
                self.MyApp.stacked_widget.setCurrentWidget(self.MyApp.dashboard_page)  
        else:
            self.MyApp.notification.set_message("Error", "Login yoki Parol xato!", "#ff0060", "error")
            self.MyApp.show_notification()
        self.gif.stop()
